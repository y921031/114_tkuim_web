...
| --- | --- | --- | --- |
| **超級管理員** | `super@test.com` | `123` | 可看見所有人資料、可刪除任何人資料 |
| **一般學員** | `mytest@test.com` | `123` | 僅能看見自己新增的資料、無法刪除他人資料 |
...
3. 確認列表出現該筆資料。
4. 按下右上角「登出」。
![admin_delete](assets/admin_delete.png)

### 2. 管理員權限測試 (Admin)

1. 使用 `super@test.com` / `123` 登入。
2. **關鍵驗收點**：列表中應該要能看到剛剛學生建立的那筆資料（證明 Admin 可視範圍為全域）。
![admin_see](assets/admin_see.png)
1. 點擊該筆資料旁的「刪除」按鈕。
2. **關鍵驗收點**：系統提示「刪除成功」，資料消失（證明 Admin 擁有最高管理權，可刪除他人資料）。
![admin_delete](assets/admin_delete.png)